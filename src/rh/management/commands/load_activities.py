    ...
    def _import_data(self):
        ...
        for cluster_id, group_data in grouped_by_cluster_data:
            ...
            for activity_type_id, group_data_1 in grouped_by_activity_type_id_data:
                activity_domain,created  = ActivityDomain.objects.get_or_create(code=activity_type_id[0],name=activity_type_id[1])
                if created:
                   activity_domain.clusters.add(cluster) 
                   activity_domain.save()

                g_by_activity_description_data = group_data_1.groupby(['activity_description_id','activity_description_name'])
                for activity_description_id, group_data_2 in g_by_activity_description_data:
                    activity_type,created  = ActivityType.objects.get_or_create(code=activity_description_id[0],name=activity_description_id[1])
                    if created:
                    ...
                    for activity_detail_id, group_data_3 in g_by_activity_details_data:
                        activity_detail,created  = ActivityDetail.objects.get_or_create(code=activity_detail_id[0],name=activity_detail_id[1])
                        if created:
