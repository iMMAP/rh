...
class Command(BaseCommand):
    ...
    def _create_activity_plans(self):
        ...
        for index, plan in enumerate(plans):
            ...
            transfer_mechanism_type = TransferMechanismType.objects.filter(name=plan.get('mpc_mechanism_type_name', '')).first()
            implement_modility_type = ImplementationModalityType.objects.filter(name=plan.get('mpc_delivery_type_name', '')).first()
