...
def create_report_target_locations(request, project, report, plan):
    ...
    if request.method == "POST":
        ...
        if location_report_form.is_valid() and report_disaggregation_formset.is_valid():
            ...
            return redirect("list_report_target_locations", project=monthly_report.project.pk, report=monthly_report.pk)
    else:
...
@login_required
def list_report_target_locations(request, project, report):
    """Create View"""
    project = get_object_or_404(Project, pk=project)
    monthly_report_instance = get_object_or_404(ProjectMonthlyReport, pk=report)
    report_locations = TargetLocationReport.objects.filter(activity_plan_report__monthly_report=report).order_by("-id")
...
@login_required
def update_report_target_locations(request, project, report, plan):
